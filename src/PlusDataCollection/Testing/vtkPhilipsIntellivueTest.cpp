...
int main(int argc, char **argv)
{
  ...
  std::string inputConfigFileName = "Testing/PlusDeviceSet_DataCollectionOnly_PhilipsIntellivue.xml";
  ...
  vtkNew<vtkPlusIntelliVue> intellivueDevice;
  intellivueDevice->SetDeviceId("SerialDevice");
  ...
  if (intellivueDevice->ReadConfiguration(configRead) != PLUS_SUCCESS)
  {
    LOG_ERROR( "Failed to read configuration"); 
    exit(EXIT_FAILURE); 
  }

  if (intellivueDevice->NotifyConfigured() != PLUS_SUCCESS)
  {
    LOG_ERROR( "Invalid configuration"); 
    exit(EXIT_FAILURE); 
  }

  intellivueDevice->CreateDefaultOutputChannel(NULL, true);
  vtkPlusDataSource* videoSource = NULL;
  if (intellivueDevice->GetFirstActiveOutputVideoSource(videoSource) != PLUS_SUCCESS)
  {
  ...
  vtkIndent indent;
  intellivueDevice->PrintSelf(std::cout, indent);
  ...
  intellivueDevice->AddObserver("WarningEvent", callbackCommand); 
  intellivueDevice->AddObserver("ErrorEvent", callbackCommand); 
  
  if (intellivueDevice->Connect() != PLUS_SUCCESS) {
    LOG_ERROR( "Failed to connect"); 
    exit(EXIT_FAILURE); 
  }

  if (intellivueDevice->StartRecording() != PLUS_SUCCESS)
  {
  ...
  if (!renderingOff) {
    ...
    viewer->SetInputConnection(intellivueDevice->GetOutputPort());   //set image to the render and window
  ...
  if (intellivueDevice->StopRecording() != PLUS_SUCCESS)
  {
    LOG_INFO("Failed to stop recording"); 
    exit(EXIT_FAILURE); 
  }

  if (intellivueDevice->Disconnect() != PLUS_SUCCESS) {
